...
export default function Excelpage() {
  ...
  const handleUpload = async () => {
    if (!userData) {
      ...
      alert(
        "No data to upload. Please upload an excel sheet in the prescribed format."
      );
      return;
    }
    ...
    for (let i = 0; i < userData.length; i++) {
      ...
      const confirmationBody = `
            The event titled "${
              userData[i].eventTitle
            }" with the following details has been approved: 
            - Category: ${userData[i].category} 
            - Convenor Name: ${userData[i].convenorName} (${
        userData[i].convenorDesignation
      }) 
            - Email: ${userData[i].mailId}, Mobile: ${userData[i].mobileNumber} 
            - Proposed Period: ${
              userData[i].proposedPeriod
            }, Duration: ${JSON.stringify(userData[i].duration)} days 
            - Financial Support (Others): ${JSON.stringify(
              userData[i].financialSupportOthers
            )}, Financial Support (SRMIST): ${JSON.stringify(
        userData[i].financialSupportSRMIST
      )} 
      ...
      try {
        const result = await axios.post("/api/proposal", {
          ...
          financialSupportOthers: JSON.stringify(
            userData[i].financialSupportOthers
          ),
          financialSupportSRMIST: JSON.stringify(
            userData[i].financialSupportSRMIST
          ),
        ...
        if (result.status === 200) {
          ...
          if (sendingEmailResult.status === 200) {
            console.log(
              "Email sent to applicant with email: " + userData[i].mailId
            );
            toast.success(
              "Email sent to applicant with email: " + userData[i].mailId
            );
            ...
            toast.error(
              "Email not sent to applicant with email: " + userData[i].mailId
            );
          }
        } else if (result.status === 203) {
          toast.error(
            `Event already exists for user ${userData[i].convenorName} with title ${userData[i].eventTitle}`
          );
        } else {
          console.error(
            "Error: Status code sent by server for accessing database is " +
              result.status
          );
      ...
      <div className="container mx-auto p-8 mt-10 bg-white shadow-lg rounded-lg">
        <div className="text-center">
          ...
          <p className="mb-8 text-gray-700">
            By choosing your excel file, you can directly add the details.
            Please make sure that the excel file is in the prescribed format.
          </p>
        </div>
